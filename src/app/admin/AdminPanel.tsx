...
/* ============================================================
   CONSTÂNCIA NA PALAVRA — Painel /admin (componente interno)
   Blindagem de 3 camadas (a mesma do molde, endurecida no FST em 11/07
   depois da primeira cliente real):
     1. page.tsx (servidor): não-admin recebe 404 — a página "não existe".
     2. Este componente re-checa a flag `admin` do /api/perfil.
     3. As APIs /api/admin/* re-checam ehAdmin no servidor (403).
   Três seções: (1) Números — o retrato do freemium em uma linha;
   (2) Liberar acesso em massa (cola e-mails, escolhe plano);
   (3) Usuários — lista todo mundo e troca/cancela o plano por pessoa.
...
// Cores na paleta Luz e Lavra. As do molde vinham do FST (vitalício laranja,
// mensal verde-água) e brigavam com a identidade: aqui o vitalício é o plano de
// destaque e a cor de destaque é o ouro, não o laranja.
const PLANO_INFO: Record<string, { nome: string; cor: string; bg: string }> = {
  free: { nome: "Grátis", cor: "#8B7A61", bg: "#F1EDE2" },
  mensal: { nome: "Mensal", cor: "#4E5A2E", bg: "#EAEEDF" },
  vitalicio: { nome: "Vitalício", cor: "#8F6D1E", bg: "#F5EAD0" },
};
...
export default function AdminPanel() {
  ...
  // Retrato do freemium. Sai de graça da lista que o painel já carregou —
  // nenhuma consulta a mais. Desde 01/09 o modelo tem porta grátis, então a
  // conversão free→pago é o número que decide se o negócio fecha (B2 da régua,
  // meta ≥3%). Um painel sem esse número é um painel cego pro modelo novo.
  const numeros = useMemo(() => {
    const total = usuarios.length;
    const pagas = usuarios.filter((u) => u.plano === "mensal" || u.plano === "vitalicio").length;
    const mensais = usuarios.filter((u) => u.plano === "mensal").length;
    const vitalicias = usuarios.filter((u) => u.plano === "vitalicio").length;
    return {
      total,
      gratis: total - pagas,
      pagas,
      mensais,
      vitalicias,
      conversao: total > 0 ? (pagas / total) * 100 : 0,
    };
  }, [usuarios]);
  ...
  if (perfil === null) {
    ...
    corpo = (
      <section style={card}>
        ...
        <p style={{ color: "#6C5C45", fontSize: 15, lineHeight: 1.55, marginBottom: 18 }}>Esta área é só para administradores logados.</p>
    ...
    corpo = (
      <section style={card}>
        ...
        <p style={{ color: "#6C5C45", fontSize: 15, lineHeight: 1.55, marginBottom: 18 }}>Esta área é só para administradores.</p>
    ...
    corpo = (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {/* ---- 1. Números: o retrato do freemium ---- */}
        <section style={card}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
            <h2 style={{ fontFamily: C.serif, fontSize: "clamp(21px,3.4vw,26px)", color: C.base, margin: 0 }}>Números</h2>
            <button
              type="button"
              onClick={carregarUsuarios}
              disabled={carregandoU}
              style={{
                font: "inherit", fontSize: 12.5, fontWeight: 600, color: C.muted,
                background: "transparent", border: `1px solid ${C.line}`, borderRadius: 9,
                padding: "5px 11px", cursor: carregandoU ? "default" : "pointer",
              }}
            >
              {carregandoU ? "Atualizando…" : "Atualizar"}
            </button>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(104px,1fr))", gap: 10, marginTop: 16 }}>
            {[
              { r: "Contas", v: String(numeros.total), d: "no total" },
              { r: "Grátis", v: String(numeros.gratis), d: "só o marcador" },
              { r: "Pagantes", v: String(numeros.pagas), d: `${numeros.mensais} mensal · ${numeros.vitalicias} vital.` },
              {
                r: "Conversão",
                v: `${numeros.conversao.toFixed(1).replace(".", ",")}%`,
                d: numeros.total === 0 ? "sem base ainda" : "free → pago",
                destaque: true,
              },
            ].map((n) => (
              <div
                key={n.r}
                style={{
                  border: `1px solid ${n.destaque ? "rgba(143,109,30,.35)" : C.line}`,
                  background: n.destaque ? "rgba(232,217,174,.28)" : "transparent",
                  borderRadius: 14, padding: "13px 14px",
                }}
              >
                <div style={{ fontSize: 10.5, letterSpacing: 1.2, textTransform: "uppercase", fontWeight: 700, color: "#8F6D1E" }}>{n.r}</div>
                <div style={{ fontFamily: C.serif, fontSize: 26, color: C.base, lineHeight: 1.1, marginTop: 6 }}>{n.v}</div>
                <div style={{ fontSize: 11.5, color: C.muted, marginTop: 3 }}>{n.d}</div>
              </div>
            ))}
          </div>

          <p style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.5, marginTop: 14 }}>
            A conversão conta cortesias liberadas por aqui como pagantes — então ela só vira
            número de negócio de verdade depois que o checkout abrir. A régua do case (B2) pede
            3% na segunda coorte, não na primeira: a primeira é feita de fãs da Elisangela e
            superestima.
          </p>
        </section>

        {/* ---- 2. Liberar acesso em massa ---- */}
        <section style={card}>
        ...
        <section style={card}>
          ...
          <p style={{ color: "#6C5C45", fontSize: 14, lineHeight: 1.5, margin: "6px 0 14px" }}>
            Veja todo mundo e troque ou cancele o plano de cada um. Muda na hora. Cancelar não expulsa ninguém: devolve a leitora para o grátis, com o marcador da Bíblia intacto.
          </p>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
                    {ACOES.map((a) => {
                      ...
                      return (
                        ...
                        >
                          {atual ? `✓ ${cancelar ? "Grátis" : a.rot}` : a.rot}
                        </button>
                      );
                    })}
                  </div>
