...
const ABAS = [
  { href: "/biblia", label: "Bíblia", emoji: "📕", pago: false },
  { href: "/ler", label: "Hoje", emoji: "🕯️", pago: true },
  { href: "/planos", label: "Planos", emoji: "🗺️", pago: true },
  { href: "/mural", label: "Mural", emoji: "💬", pago: true },
  { href: "/conta", label: "Conta", emoji: "👤", pago: false },
];
  ...
  return (
    <nav className="an-nav">
        ...
        <div className="an-abas">
          {ABAS.map((a) => {
            ...
            return (
              ...
              >
                <span className="an-em" aria-hidden>
                  {a.emoji}
                </span>
                <span className="an-lb">{a.label}</span>
                {travada && <Lock size={11} className="an-cad" aria-hidden />}
              </Link>
            );
          })}
        </div>
      ...
      <style jsx>{`
        ...
        .an-aba :global(.an-em){font-size:15px;filter:grayscale(.3);opacity:.75}
        .an-aba:hover{color:var(--base)}
        .an-aba.on{color:var(--coral);background:color-mix(in srgb,var(--coral) 11%,transparent)}
        .an-aba.on :global(.an-em){filter:none;opacity:1}
        .an-aba.lock{opacity:.62}
        .an-aba.lock :global(.an-em){filter:grayscale(1);opacity:.5}
        .an-aba.lock:hover{opacity:1;color:var(--ouro)}
        .an-aba :global(.an-cad){color:var(--ouro);flex:none}
